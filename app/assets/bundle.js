...
(function () {
	...
	var btnSVG = btn.querySelector('.btn__svg');
	var btnInner = btn.querySelector('.btn__inner');
	var speed = 300;
	...
	function getEntryPos(e) {
		var elCoords = btn.getBoundingClientRect();
...
// Dom Elements
var items = document.querySelectorAll('.gooey__item');
var rectangle = document.querySelector('.gooey__rectangle');
var container = document.querySelector('.gooey');
var active = document.querySelector('.active');
var itemsArray = Array.from(items);

// Co-ords
var rectCoords = active.getBoundingClientRect();
var containerCoords = container.getBoundingClientRect();
var containerOffset = window.getComputedStyle(container, null).getPropertyValue('padding-left').replace(/\D/g, '');

// Values + Other
var transVal = void 0;
var prevPosition = getArrayPosition(active);
var transDur = 0.4;
var scaleHeight = 0.25;
var scaleWidth = 0.7;
...
function getCoords(el) {
	var thisCoords = el.getBoundingClientRect();
	var newCoords = {
...
function gooeyHandler(e) {
	...
	var currentPosition = getArrayPosition(this);
	...
	rectangle.addEventListener('transitionend', function () {
		return afterTransition(_this, newCoords);
	}, { once: true });
}
...
items.forEach(function (item) {
	return item.addEventListener('click', gooeyHandler);
});
...
var wrapper = document.querySelector('.parallax-card__wrapper');
var content = document.querySelector('.parallax-card__content-animation');
var bg = document.querySelector('.parallax-card__bg');
var shiny = document.querySelector('.parallax-card__shiny');
...
function getMousePos(e) {
	var bgCoords = this.getBoundingClientRect();
	var coords = {
...
function createObjectsByApplyingMathetmaticalComputations(coords) {
	var percentageY = coords.posY / coords.height;
	var percentageX = coords.posX / coords.width;
	var values = {
		rotate: {
...
var transition = document.querySelector('[data-js="transition"]');
var transitionBottom = document.querySelector('[data-js="transition.bottom"]');
var container = document.querySelector('[data-js="transition.container"]');
...
anchors.forEach(function (anchor) {
	return anchor.addEventListener('click', triggerTransition);
});
...
// Javascript imports
// -------------------
//const javscsripts = require('./js/react-test.jsx');
var Snap = __webpack_require__(6);
var gooey = __webpack_require__(1);
var snapTest = __webpack_require__(3);
var parallaxCard = __webpack_require__(2);
var parallaxScroll = __webpack_require__(10);
var btn = __webpack_require__(0);
var transition = __webpack_require__(4);
var mousePos = __webpack_require__(12);
...
(function () {
	var mousePos = document.querySelector('.mousePos');
	var items = document.querySelectorAll('[data-js="parallax.item"]');
	var innerHeight = window.innerHeight;
	var prevScroll = 0;

	function handler(e) {
		var scrollY = window.scrollY;
		var direction = getScrollDirection(scrollY);
		items.forEach(function (item) {
			var topPercent = ((item.offsetTop - scrollY) / innerHeight * 100).toFixed(3);
	...
	function applyTransformations(item, percent) {
		var img = item.querySelector('.parallax-scroll__img');
